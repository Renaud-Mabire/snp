...
snp_graph <- function(simulations,
  ...
  if (network_community_detection){
    list_NetworkCom <- lapply(
      Fit_Ising_list,
      function(x)
        graph_from_adjacency_matrix(abs(x$weiadj), "undirected", weighted = TRUE, add.colnames = FALSE) %>% # igraph creation from IsingFit matrix
        cluster_walktrap() %>%
  ...
  if (network_community_detection){
    ...
    for (i in 1:n_plots) {
      ...
      list_graph[[i]] <- qgraph(simulations[["simulations"]][[i]][["Fit_sample"]][["weiadj"]],
                                ...
                                fade = fade,
                                legend = legend,
                                title = paste0("Attitude - Graphe" , i),
                                title.cex = 1.25,
