  easy_ising_simulations <- function(df,
    ...
    # Check if thresholds_IsingSampler is a numeric vector
    if (!is.numeric(thresholds_IsingSampler)) {
      stop("thresholds_IsingSampler must be a numeric vector.")
    }
    ...
    # Convert thresholds_IsingSampler to a list if not already
    if (!is.list(thresholds_IsingSampler)) {
      thresholds_IsingSampler <- as.list(thresholds_IsingSampler)
    }
    ...
    if (Fit_Ising_thresholds) {
      list_thresholds <- lapply(thresholds_IsingSampler, function(x) {
        ifelse(seq_along(df) %in% Position_in_the_df_of_the_modified_nodes, x, Fit_Ising$thresholds[seq_along(df)])
      })
    } else {
      list_thresholds <- lapply(thresholds_IsingSampler, function(x) ifelse(seq_along(df) %in% Position_in_the_df_of_the_modified_nodes, x, 0))
    }
    ...
    sample_list <- lapply(list_thresholds, function(x) IsingSampler(n, SimInput$graph, thresholds = x, beta = beta, responses = c(-1L, 1L)))
    # Recode the -1 by 0 and store the result in a new list
    sample_list_rec <- lapply(sample_list, function(x) ifelse(x == -1, 0, x))
    # Estimation of an Ising network for each of the samples
    Fit_sample <- lapply(sample_list_rec, function(x) IsingFit(x, plot = FALSE))
    # Calculation of individual scores for each of the networks
    sum_score_list <- lapply(sample_list, function(x) apply(x, 1, sum))

    # Calculation of mean, median, sd, min and max of each of the networks
    list_mean <- sapply(sum_score_list, mean)
    list_median <- sapply(sum_score_list, median)
    list_sd <- sapply(sum_score_list, sd)
    list_min <- sapply(sum_score_list, min)
    list_max <- sapply(sum_score_list, max)
    #
    if (is.numeric(nodes_to_influence)) {
      easy_ising_simulations_obj <- list(
        ...
        sample_list_rec = sample_list_rec,
        sum_score_list = sum_score_list,
        list_mean = list_mean,
